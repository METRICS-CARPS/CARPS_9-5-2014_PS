...
Inconsistency 1: When attributing trustworthiness, there was no difference between the consistency of 3-4 year olds and 5-6 year olds. (p=.0465) 

Inconsistency 2: When looking at the consistency of all other groups "which exhibited equivalent consistency (all ps>.23)" for trustworthiness, the 5-6 year olds vs. the 7-10 year olds, p=.0378. I am not sure if this was not marked as significant because of the Sidak correction for multiple comparisons, but it is not greater than .23. 

Inconsistency 3: When looking at the attribute of dominance, the 5-6 year olds were significantly different than the adults (p=.0047) and the 7-10 year olds compared to the adults had a p=.0532, which again is inconsistent with what the authors stated. 

It is unclear to me whether these inconsistencies are a) a result of running this test in R using this package as opposed to in another software; b) due to replicator error in calculating or interpreting outputs or; c) indicative of true errors. 
```{r final anovas}
...
summary(aov(SMART ~ Age.Group, data=d)) #this is not mentioned
...
#inconsistency 1
compareValues(reportedValue = .01, obtainedValue = .0465, isP = T)
#decision error and major numerical error

#inconsistency 2
compareValues(reportedValue = .23, obtainedValue = .0378, isP = T)
#decision error and major numerical error

#inconsistency 3
compareValues(reportedValue = .23, obtainedValue = .0047, isP = T)
#decision error and major numerical error
compareValues(reportedValue = .23, obtainedValue = .0532, isP = T)
#decision error and major numerical error
...
Here, we run into the same issue we ran into before: without more information about which specific tests were used to calculate "consistency," we would need to engage in "lengthy guesswork" to determine what tests to use - although the authors state that an anova with post-hoc tests with Sidak corrections were used, this test (as far as I am aware) does not yield an effect size, which needs to be calculated separately. Thus, there is insufficient information regarding what statistical tests were used to calculate the effect sizes. 
